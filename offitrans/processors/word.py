...
class WordProcessor(BaseProcessor):
        ...
        try:
            ...
            for para_idx, paragraph in enumerate(doc.paragraphs):
                if paragraph.text.strip():
                    ...
                    logger.debug(f"Extracted paragraph {para_idx}: '{paragraph.text[:50]}...'")
                ...
                for row_idx, row in enumerate(table.rows):
                    for cell_idx, cell in enumerate(row.cells):
                        if cell.text.strip():
                            ...
                            logger.debug(f"Extracted table cell [{table_idx}][{row_idx}][{cell_idx}]: '{cell.text[:50]}...'")
        ...
        try:
            ...
            for para_idx, para_info in paragraph_translations.items():
                if para_idx < len(doc.paragraphs):
                    ...
                    self._apply_run_format(run, para_info.get('format', {}), target_language)
                    
                    logger.debug(f"Applied translation to paragraph {para_idx}")
            
            # Apply table cell translations
            for (table_idx, row_idx, cell_idx), translated_text in table_translations.items():
                if (table_idx < len(doc.tables) and 
                    ...
                    logger.debug(f"Applied translation to table cell [{table_idx}][{row_idx}][{cell_idx}]")
    ...
    def _apply_run_format(self, run, format_info: Dict[str, Any], target_language: str = "en") -> None:
        """
        ...
        try:
            ...
            if format_info.get('font_size'):
                ...
                if original_size:
                    adjusted_size = max(Pt(6), Pt(original_size.pt * self.font_size_adjustment))
...
def docx_translate(input_file: str, output_file: str, target_language: str = "en") -> bool:
    """
