...
class BatchProcessor:
    ...
    def process_file(
        self, input_file: str, output_file: str, target_language: str = "en"
    ) -> Dict[str, Any]:
        """
    ...
    def process_files_sequential(
        self, file_pairs: List[tuple], target_language: str = "en"
    ) -> List[Dict[str, Any]]:
        """
    ...
    def process_files_parallel(
        self, file_pairs: List[tuple], target_language: str = "en", max_workers: int = 3
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        print(
            f"Processing {len(file_pairs)} files in parallel with {max_workers} workers..."
        )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    print(
                        f"{status} {input_file} -> {output_file} ({result['processing_time']:.1f}s)"
                    )
                except Exception as e:
    ...
    def print_summary(self):
        ...
        print(
            f"Success rate: {(self.stats['successful_files'] / max(1, self.stats['total_files']) * 100):.1f}%"
        )
        print(
            f"Total processing time: {self.stats['total_processing_time']:.1f} seconds"
        )

        if self.stats["successful_files"] > 0:
            avg_time = (
                self.stats["total_processing_time"] / self.stats["successful_files"]
            )
        ...
        if successful_files:
            ...
            for result in successful_files:
                print(
                    f"   - {result['input_file']} ({result['processing_time']:.1f}s, {result['texts_translated']} texts)"
                )
...
def create_sample_files():
    ...
    try:
        ...
        sample_data = [
            ...
            (
                "sample2.xlsx",
                ["Welcome", "Batch processing function", "Very convenient"],
            ),
            (
                "sample3.xlsx",
                [
                    "Test file",
                    "Contains various content",
                    "Number 123",
    ...
    return [
        sample_dir / f
        for f in ["sample1.xlsx", "sample2.xlsx", "sample3.xlsx", "sample4.xlsx"]
    ]
...
def demo_sequential_processing():
    ...
    for input_file in sample_files:
        output_file = (
            input_file.parent / f"{input_file.stem}_translated_seq{input_file.suffix}"
        )
...
def demo_parallel_processing():
    ...
    for input_file in sample_files:
        output_file = (
            input_file.parent / f"{input_file.stem}_translated_par{input_file.suffix}"
        )
        file_pairs.append((str(input_file), str(output_file)))

    # Create processor with settings optimized for parallel processing
    config = Config()
    config.translator.max_workers = (
        1  # Reduce per-processor workers for parallel processing
    )
    config.cache.enabled = True
    ...
    results = processor.process_files_parallel(
        file_pairs, target_language="en", max_workers=3
    )
...
def demo_mixed_file_types():
    ...
    results = processor.process_files_parallel(
        file_pairs, target_language="en", max_workers=2
    )
...
def demo_progress_monitoring():
    ...
    for i, input_file in enumerate(sample_files, 1):
        output_file = (
            input_file.parent / f"{input_file.stem}_monitored{input_file.suffix}"
        )
